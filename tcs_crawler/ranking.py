...
CLAIM_PATTERNS: list[tuple[str, float, str]] = [
    (r"\bresolv(e|es|ing)\b.{0,40}\b(conjecture|open (problem|question))|"
     ...
     r"\banswer(s|ing)? .{0,30}\bopen (problem|question)", 3.0, "resolves an open problem"),
    (r"\bfirst\b.{0,30}\b(algorithm|bound|construction|proof|separation|result)|"
     r"\bwe give the first|\bthe first (polynomial|sub|near|constant|nontrivial)", 2.0, "first of its kind"),
    (r"\boptimal\b|\btight(ly)? (bound|analysis)|\bmatching lower bound|\bnearly[- ]optimal", 1.5, "optimal or tight bound"),
    (r"\bimprov(e|es|ing|ed)\b.{0,40}\b(bound|running time|approximation|complexity|factor)|"
     r"\bbreak(s|ing)? the\b|\bbeat(s|ing)? the\b", 1.2, "improves a known bound"),
    (r"\blower bound|\bhardness\b|\binapproximab|\bimpossibility\b|\bseparation\b", 1.0, "lower bound or impossibility"),
    (r"\bdisprov(e|es|ing)\b|\bcounterexample\b|\brefut(e|es|ing)\b", 2.0, "disproof"),
]
...
def score(paper: ArxivPaper, prolific: dict[str, int]) -> tuple[float, dict]:
    ...
    for rx, w, label in _CLAIMS:
        if rx.search(text):
            ...
            notes.append(label)
    if claim:
        parts["claim strength"] = round(min(4.0, claim), 2)

    # 5. 已被会议/期刊接收，是同行评议给过的信号
    if paper.journal_ref or re.search(r"\b(accepted|to appear)\b", paper.comment, re.IGNORECASE):
        parts["peer-reviewed"] = 1.5
        notes.append("comment or journal-ref says accepted")

    if NEGATIVE.search(paper.title):
        parts["not a new result"] = -3.0
        notes.append("title suggests this is not a new result")
